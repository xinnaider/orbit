...
mod tests {
    ...
    #[test]
    fn should_return_null_ssh_fields_for_local_session() {
        let mut t = TestCase::new("should_return_null_ssh_fields_for_local_session");
        t.phase("Act");
        let db = make_db();
        let id = db
            .create_session(None, None, "/tmp/proj", "ignore", None, None, None)
        ...
        t.phase("Assert");
        let s = db.get_session(id).expect("get failed").expect("missing");
        t.none("ssh_host is None for local session", &s.ssh_host);
        t.none("ssh_user is None for local session", &s.ssh_user);
    }
