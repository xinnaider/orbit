...
mod tests {
    ...
    fn shipped_registry_should_dispatch_line_processors() {
        ...
        t.ok(
            "claude processor",
            registry.line_processor_for("claude-code") == claude.line_processor(),
        );
        t.ok(
            "codex processor",
            registry.line_processor_for("codex") == codex.line_processor(),
        );
        t.ok(
            "opencode processor",
            registry.line_processor_for("opencode") == opencode.line_processor(),
        );
        t.ok(
            "openrouter falls back to opencode parser",
            registry.line_processor_for("openrouter") == opencode.line_processor(),
        );
        t.ok(
            "legacy gemini uses acp parser",
            registry.line_processor_for("gemini-cli") == acp::process_acp_line,
        );
    }
