...
const MOCK_SESSIONS: Session[] = [
  {
    id: 1, projectId: 1, name: 'fix auth bug',
    status: 'running' as const, permissionMode: 'ignore',
    model: 'claude-sonnet-4-6', pid: 12345,
    cwd: 'C:\\Users\\dev\\api-server', projectName: 'api-server',
    gitBranch: 'main',
    tokens: { input: 24800, output: 3400, cacheRead: 12000, cacheWrite: 800 },
    contextPercent: 14.2, pendingApproval: null,
    miniLog: [
  ...
  {
    id: 2, projectId: 2, name: null,
    status: 'waiting' as const, permissionMode: 'approve',
    model: 'claude-opus-4-6', pid: 23456,
    cwd: 'C:\\Users\\dev\\dashboard', projectName: 'dashboard',
    gitBranch: 'feat/redesign',
    tokens: { input: 89200, output: 12300, cacheRead: 45000, cacheWrite: 3200 },
    contextPercent: 51.3, pendingApproval: 'Allow Bash to run: rm -rf dist',
    miniLog: null,
    createdAt: new Date(Date.now() - 60000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 3, projectId: 3, name: 'add tests',
    status: 'completed', permissionMode: 'ignore',
    model: 'claude-haiku-4-5-20251001', pid: null,
    cwd: 'C:\\Users\\dev\\utils', projectName: 'utils-lib',
    gitBranch: null, tokens: { input: 5200, output: 980, cacheRead: 2100, cacheWrite: 120 },
    contextPercent: 3.1, pendingApproval: null, miniLog: null,
...
const MOCK_JOURNAL: Record<number, JournalEntry[]> = {
  1: [
    {
      sessionId: '1', timestamp: new Date(Date.now() - 120000).toISOString(),
      entryType: 'user', text: 'Fix the JWT token refresh bug in auth.ts — tokens expire too early',
      thinking: null, thinkingDuration: null, tool: null, toolInput: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 115000).toISOString(),
      entryType: 'thinking',
      thinking: "Let me analyze the JWT token refresh logic. The issue is likely in the expiry calculation...\n\nI need to check:\n1. How the token expiry is set\n2. Where the refresh logic is\n3. Whether there's a race condition",
      thinkingDuration: 3.2, text: null, tool: null, toolInput: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 111000).toISOString(),
      entryType: 'assistant',
    ...
    {
      sessionId: '1', timestamp: new Date(Date.now() - 110000).toISOString(),
      entryType: 'toolCall', tool: 'Read',
      toolInput: { file_path: 'src/auth/auth.ts' },
      text: null, thinking: null, thinkingDuration: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 109000).toISOString(),
      entryType: 'toolResult', tool: null, toolInput: null,
      ...
      text: null, thinking: null, thinkingDuration: null,
      exitCode: 0, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 105000).toISOString(),
      entryType: 'toolCall', tool: 'Edit',
      toolInput: { file_path: 'src/auth/auth.ts', old_string: "expiresIn: '1s'", new_string: "expiresIn: '1h'" },
      text: null, thinking: null, thinkingDuration: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 104000).toISOString(),
      entryType: 'toolResult', tool: null, toolInput: null,
      output: 'File updated successfully.',
      text: null, thinking: null, thinkingDuration: null,
      exitCode: 0, linesChanged: { added: 1, removed: 1 },
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 100000).toISOString(),
      entryType: 'toolCall', tool: 'Bash',
      toolInput: { command: 'npm test -- --testPathPattern=auth' },
      text: null, thinking: null, thinkingDuration: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 95000).toISOString(),
      entryType: 'toolResult', tool: null, toolInput: null,
      output: 'PASS src/auth/auth.test.ts\n  ✓ refreshes token with correct expiry (23ms)\n  ✓ rejects expired tokens (12ms)\n\nTest Suites: 1 passed, 1 total',
      text: null, thinking: null, thinkingDuration: null,
      exitCode: 0, linesChanged: null,
    },
    {
      sessionId: '1', timestamp: new Date(Date.now() - 90000).toISOString(),
      entryType: 'assistant',
  ...
  2: [
    {
      sessionId: '2', timestamp: new Date(Date.now() - 30000).toISOString(),
      entryType: 'user', text: 'Rebuild the dashboard frontend from scratch',
      thinking: null, thinkingDuration: null, tool: null, toolInput: null,
      output: null, exitCode: null, linesChanged: null,
    },
    {
      sessionId: '2', timestamp: new Date(Date.now() - 25000).toISOString(),
      entryType: 'toolCall', tool: 'Bash',
  ...
  switch (cmd) {
    ...
    case 'create_session': {
      ...
      const newSession: Session = {
        ...
        projectName: String(args?.projectPath ?? '').split(/[\\/]/).pop() ?? 'project',
        gitBranch: null, tokens: null, contextPercent: null,
        pendingApproval: null, miniLog: null,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      ...
      setTimeout(() => {
        sessions = sessions.map(s => s.id === id ? { ...s, status: 'running', pid: 10000 + id } : s);
...
function simulateClaudeResponse(sessionId: number, userMsg: string) {
  ...
  setTimeout(() => {
    const thinkEntry: JournalEntry = {
      sessionId: String(sessionId), timestamp: new Date().toISOString(),
      entryType: 'thinking',
      thinking: `Processing request: "${userMsg}"\n\nI should analyze this carefully and provide a helpful response.`,
      thinkingDuration: 1.8,
      text: null, tool: null, toolInput: null, output: null, exitCode: null, linesChanged: null,
    };
  ...
  setTimeout(() => {
    const toolEntry: JournalEntry = {
      ...
      text: null, thinking: null, thinkingDuration: null, output: null, exitCode: null, linesChanged: null,
    };
  ...
  setTimeout(() => {
    const resultEntry: JournalEntry = {
      ...
      text: null, thinking: null, thinkingDuration: null, exitCode: 0, linesChanged: null,
    };
  ...
  setTimeout(() => {
    ...
    mockEmit('session:state', makeStateEvent(sessionId, 'idle', {
